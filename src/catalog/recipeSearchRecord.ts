...
import { formatStackIdentity, parseSearchQuery, type SearchMode } from '../lib/recipeHelpers'
...
export function recipeSearchRecordMatchesQuery(
  ...
  if (parsedQuery.isExact) {
    return recordMatchesExactQuery(record, parsedQuery.text, mode)
  }

  return recordMatchesFuzzyQuery(record, parsedQuery.text, mode)
}

function buildStackSearchRecords(stacks: readonly ExportStack[]): readonly StackSearchRecord[] {
  return Object.freeze(stacks.map((stack) => buildStackSearchRecord(stack)))
}
...
function recordMatchesFuzzyQuery(
  record: RecipeSearchRecord,
  query: string,
  mode: SearchMode,
): boolean {
  switch (mode) {
    case 'inputs':
      return record.inputs.some((stack) => stackMatchesFuzzyText(stack, query))

    case 'outputs':
      return record.outputs.some((stack) => stackMatchesFuzzyText(stack, query))

    case 'machines':
      return machineMatchesFuzzyQuery(record, query)

    case 'ids':
      return (
        record.recipeIdText.includes(query) ||
        record.machineIdText.includes(query) ||
        record.inputs.some((stack) => stackMatchesFuzzyIdentity(stack, query)) ||
        record.tools.some((stack) => stackMatchesFuzzyIdentity(stack, query)) ||
        record.outputs.some((stack) => stackMatchesFuzzyIdentity(stack, query))
      )

    case 'all':
      return (
        record.recipeIdText.includes(query) ||
        machineMatchesFuzzyQuery(record, query) ||
        record.inputs.some((stack) => stackMatchesFuzzyText(stack, query)) ||
        record.tools.some((stack) => stackMatchesFuzzyText(stack, query)) ||
        record.outputs.some((stack) => stackMatchesFuzzyText(stack, query))
      )
  }
}

function recordMatchesExactQuery(
  record: RecipeSearchRecord,
  query: string,
  mode: SearchMode,
): boolean {
  switch (mode) {
    case 'inputs':
      return record.inputs.some((stack) => stackMatchesExactText(stack, query))

    case 'outputs':
      return record.outputs.some((stack) => stackMatchesExactText(stack, query))

    case 'machines':
      return machineMatchesExactQuery(record, query)

    case 'ids':
      return (
        record.recipeIdText === query ||
        record.machineIdText === query ||
        record.inputs.some((stack) => stackMatchesExactIdentity(stack, query)) ||
        record.tools.some((stack) => stackMatchesExactIdentity(stack, query)) ||
        record.outputs.some((stack) => stackMatchesExactIdentity(stack, query))
      )
    ...
    case 'all':
      return (
        ...
        record.inputs.some((stack) => stackMatchesExactText(stack, query)) ||
        record.tools.some((stack) => stackMatchesExactText(stack, query)) ||
        record.outputs.some((stack) => stackMatchesExactText(stack, query))
      )
  }
}
