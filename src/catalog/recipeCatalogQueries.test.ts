...
describe('recipe catalog stack queries', () => {
  it('resolves recipes that produce a stack', () => {
    ...
    const producers = getProducerRecipesForStack(catalog, output)
  ...
  it('resolves recipes that consume a stack', () => {
    ...
    const consumers = getConsumerRecipesForStack(catalog, input)
  ...
  it('does not treat tools as consumed inputs', () => {
    ...
    expect(getConsumerRecipesForStack(catalog, tool)).toEqual([])
  })
  ...
  it('returns an empty result for an unknown stack', () => {
    ...
    expect(getProducerRecipesForStack(catalog, unknownStack)).toEqual([])

    expect(getConsumerRecipesForStack(catalog, unknownStack)).toEqual([])
  })
})
...
describe('recipe catalog machine queries', () => {
  it('resolves recipes belonging to a machine', () => {
    ...
    const machineRecipes = getRecipesForMachine(catalog, recipe.machine.id)

    expect(machineRecipes).toEqual([recipe, secondRecipe])
  ...
  it('returns an empty result for an unknown machine', () => {
    ...
    expect(getRecipesForMachine(catalog, 'test:missing-machine')).toEqual([])
  })
  ...
  it('derives machine counts in descending order', () => {
    ...
    document.recipes.push(sameMachineRecipe, otherMachineRecipe)
...
describe('recipe catalog search queries', () => {
  it('searches only the supplied candidate recipes', () => {
    ...
    expect(searchRecipes(catalog, document.recipes, 'lava', 'outputs')).toEqual([secondRecipe])

    expect(searchRecipes(catalog, [recipe], 'lava', 'outputs')).toEqual([])
  })
  ...
  it('rejects candidates without search records', () => {
    ...
    expect(() => searchRecipes(brokenCatalog, [recipe], 'clay', 'inputs')).toThrow(
