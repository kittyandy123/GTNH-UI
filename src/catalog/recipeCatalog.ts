...
import { getStackKey, type StackKey } from './stackKey'
import { buildRecipeSearchRecord, type RecipeSearchRecord } from './recipeSearchRecord'
...
export function buildRecipeCatalog(document: NormalizedExportDocument): RecipeCatalog {
  ...
  for (const recipe of document.recipes) {
    if (recipesById.has(recipe.id)) {
      throw new Error(`Duplicate recipe ID in catalog: ${recipe.id}`)
    }

    allRecipeIds.push(recipe.id)
    recipesById.set(recipe.id, recipe)

    searchRecordsByRecipeId.set(recipe.id, buildRecipeSearchRecord(recipe))

    addRecipeId(recipeIdsByMachine, recipe.machine.id, recipe.id)

    indexStacks(producerRecipeIdsByStackKey, recipe.outputs, recipe.id)

    indexStacks(consumerRecipeIdsByStackKey, recipe.inputs, recipe.id)

    indexStacks(toolRecipeIdsByStackKey, recipe.tools, recipe.id)
  }
...
function indexStacks(
  index: Map<StackKey, Set<string>>,
  stacks: readonly ExportStack[],
  recipeId: string,
): void {
...
function finalizeRecipeIdIndex<Key>(
  index: Map<Key, Set<string>>,
): ReadonlyMap<Key, readonly string[]> {
  return new Map(Array.from(index, ([key, recipeIds]) => [key, Object.freeze([...recipeIds])]))
}
