...
describe('getStackKey', () => {
  it('distinguishes item stacks by metadata', () => {
    ...
    expect(firstKey).toBe('item:minecraft:clay_ball:0')

    expect(secondKey).toBe('item:minecraft:clay_ball:1')
...
describe('buildRecipeCatalog', () => {
  it('indexes recipes by recipe ID', () => {
    ...
    expect(catalog.allRecipeIds).toEqual([recipe.id])
  ...
  it('indexes recipes by machine ID', () => {
    ...
    expect(catalog.recipeIdsByMachine.get(recipe.machine.id)).toEqual([recipe.id])
  })
  ...
  it('indexes inputs, outputs, and tools without duplicating recipe IDs', () => {
    ...
    const consumerIds = catalog.consumerRecipeIdsByStackKey.get(getStackKey(input))

    const producerIds = catalog.producerRecipeIdsByStackKey.get(getStackKey(output))

    const toolRecipeIds = catalog.toolRecipeIdsByStackKey.get(getStackKey(tool))

    expect(consumerIds).toEqual([recipe.id])

    expect(producerIds).toEqual([recipe.id])

    expect(toolRecipeIds).toEqual([recipe.id])
  })
  ...
  it('rejects duplicate recipe IDs', () => {
    ...
    }).toThrow(`Duplicate recipe ID in catalog: ${recipe.id}`)
  })
})
