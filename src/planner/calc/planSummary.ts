...
export type PlannerStackRateStatus =
  'unresolved' | 'limiting' | 'short' | 'excess' | 'satisfied' | 'target' | 'byproduct'
...
export function computePlannerDraftSummary(
...
export function computePlannerPlanSummary(
  plan: PlannerPlan,
  catalog: PlannerRecipeCatalog,
): PlannerPlanSummary {
...
function computeRecipeNodeSummary(
  plan: PlannerPlan,
  rootNode: PlannerNode,
  recipe: NormalizedExportRecipe,
): PlannerPlanSummary {
  ...
  const targetRatePerSecond = targetOutput ? targetOutput.amount * operationsPerSecond : undefined
  ...
  const outputRates = recipe.outputs.map((stack, index) =>
    createStackRate({
      ...
      role: 'output',
      cyclesPerSecond: operationsPerSecond,
      constraintRatePerSecond: getConstraintRate(constraintsByRoleAndStack, 'output-wanted', stack),
      constraintRole: 'output-wanted',
      isTargetOutput: index === targetOutputIndex,
    }),
  )

  const targetOutputRates = outputRates.filter((rate) => rate.stackIndex === targetOutputIndex)
  const byproductRates = outputRates.filter((rate) => rate.stackIndex !== targetOutputIndex)
...
function solveQuickPlan(
  recipe: NormalizedExportRecipe,
  settings: PlannerNodeQuickPlanSettings,
  constraintsByRoleAndStack: Map<string, number>,
): QuickPlanSolveResult {
  ...
  if (settings.machineCountMode === 'fixed') {
    ...
    return {
      ...
      limitingStackKey: inputConstraintIsLimiting ? constraintResult.limitingStackKey : undefined,
    }
  }
...
function getBaseStackRatePerSecond(
  recipe: NormalizedExportRecipe,
  stack: ExportStack | undefined,
): number | undefined {
...
function getCyclesFromInputConstraints(
  recipe: NormalizedExportRecipe,
  constraintsByRoleAndStack: Map<string, number>,
): ConstraintCycleResult | undefined {
  let limitingCyclesPerSecond: number | undefined
  let limitingStackKey: StackKey | undefined

  recipe.inputs.forEach((stack) => {
    const providedRate = getConstraintRate(constraintsByRoleAndStack, 'input-provided', stack)
    ...
    if (
      limitingCyclesPerSecond === undefined ||
      possibleCyclesPerSecond < limitingCyclesPerSecond
    ) {
...
function getCyclesFromOutputConstraints(
  recipe: NormalizedExportRecipe,
  constraintsByRoleAndStack: Map<string, number>,
): ConstraintCycleResult | undefined {
  ...
  recipe.outputs.forEach((stack) => {
    const wantedRate = getConstraintRate(constraintsByRoleAndStack, 'output-wanted', stack)
    ...
    if (
      requiredCyclesPerSecond == undefined ||
      stackRequiredCyclesPerSecond > requiredCyclesPerSecond
    ) {
...
function getPowerEstimate(
  recipe: NormalizedExportRecipe,
  exactMachineCount: number | undefined,
  roundedMachineCount: number | undefined,
): PlannerPowerEstimate | undefined {
