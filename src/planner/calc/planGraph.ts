...
function createStackNode(
  prefix: string,
  rate: PlannerStackRate,
  index: number,
  status: 'unresolved-input' | 'target-output' | 'byproduct',
): VisualGraphNode {
  const metrics: Record<string, number | string> = {
    status: rate.status,
    ratePerSecond: rate.ratePerSecond,
...
function createGraphOverlays(
  ...
  if (unresolvedInputNodeIds.length > 0) {
    overlays.push({
      ...
      edgeIds: edges.filter((edge) => edge.status === 'short').map((edge) => edge.id),
      metrics: {
  ...
  if (byproductNodeIds.length > 0) {
    overlays.push({
      ...
      edgeIds: edges.filter((edge) => edge.status === 'excess').map((edge) => edge.id),
      metrics: {
