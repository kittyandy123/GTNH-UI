...
export function setPlannerDraftTargetRatePerSecond(
  draft: PlannerDraft,
  targetRatePerSecond: number | undefined,
...
export function setPlannerDraftTargetOutputIndex(
  draft: PlannerDraft,
  targetOutputIndex: number,
  recipe?: NormalizedExportRecipe,
): PlannerDraft {
  return updatePlannerDraftRootNode(draft, (rootNode) => {
    const normalizedTargetOutputIndex = normalizeTargetOutputIndex(targetOutputIndex, recipe)
...
function getFallbackTargetRatePerSecond(
  draft: PlannerDraft,
  recipe: NormalizedExportRecipe | undefined,
): number {
  const rootNode = getPlannerDraftRootNode(draft)

  if (recipe) {
    return getDefaultRecipeTargetRatePerSecond(recipe, rootNode?.targetOutputIndex ?? 0)
  }
...
function normalizeTargetOutputIndex(
  targetOutputIndex: number,
  recipe: NormalizedExportRecipe | undefined,
): number {
...
function updatePlannerDraftRootNode(
  draft: PlannerDraft,
  updateRootNode: (rootNode: PlannerNode) => PlannerNode,
): PlannerDraft {
  const rootNode = getPlannerDraftRootNode(draft)
...
export function setPlannerDraftMachineCountMode(
  draft: PlannerDraft,
    ...
    quickPlanSettings: {
      ...
      fixedMachineCount:
        machineCountMode === 'fixed'
          ? (normalizePositiveMachineCount(rootNode.quickPlanSettings?.fixedMachineCount) ?? 1)
          : rootNode.quickPlanSettings?.fixedMachineCount,
    },
  }))
}

export function setPlannerDraftFixedMachineCount(
  draft: PlannerDraft,
  fixedMachineCount: number | undefined,
): PlannerDraft {
  return updatePlannerDraftRootNode(draft, (rootNode) => ({
    ...rootNode,
    quickPlanSettings: {
...
export function setPlannerDraftConstraintSolveMode(
  draft: PlannerDraft,
...
export function setPlannerDraftStackConstraintRate(
  draft: PlannerDraft,
  stackKey: StackKey,
