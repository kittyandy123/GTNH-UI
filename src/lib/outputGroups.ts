...
export function buildOutputGroups(
  ...
  return Array.from(groups.values()).sort((left, right) => compareOutputGroups(left, right, query))
}

function compareOutputGroups(
  left: OutputRecipeGroup,
  right: OutputRecipeGroup,
  query: string,
): number {
  const relevanceComparison =
    getOutputGroupRelevance(right, query) - getOutputGroupRelevance(left, query)
...
function getOutputGroupRelevance(group: OutputRecipeGroup, query: string): number {
  ...
  const values = [group.output.displayName, group.output.id, formatStackIdentity(group.output)].map(
