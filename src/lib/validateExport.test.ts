...
describe('validateExportDocument', () => {
  it('accepts the exporter representative schema-v2 fixture', () => {
    expect(validateExportDocument(representativeExport)).toBe(representativeExport)
  })
  ...
  it('rejects unsupported schema versions with a clear error', () => {
    ...
    expect(() => validateExportDocument(unsupportedExport)).toThrow(
  ...
  it('accepts schema-v2 exports with classified raw negative durations', () => {
    ...
    expect(validateExportDocument(negativeDurationExport)).toBe(negativeDurationExport)
  })
  ...
  it('rejects malformed schema-v2 documents', () => {
    ...
    expect(() => validateExportDocument(malformedExport)).toThrow(
      'Recipe export failed schema-v2 validation:',
    )
  })
})
