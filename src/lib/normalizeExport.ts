...
export function normalizeExportDocument(document: ExportDocument): NormalizedExportDocument {
  const recipes = document.recipes.map((recipe) => normalizedRecipe(recipe, document.schemaVersion))

  return {
    ...document,
    diagnostics: normalizePlanningDiagnostics(document.diagnostics, recipes),
    recipes,
  }
}
...
function normalizeRecipePlanning(recipe: ExportRecipe): RecipePlanningInfo | undefined {
  const issues = new Set<RecipePlanningIssue>(recipe.planning?.issues ?? [])
...
function normalizePlanningDiagnostics(
  diagnostics: ExportDiagnostics,
  recipes: NormalizedExportRecipe[],
): ExportDiagnostics {
  const nonPlannableRecipes = recipes.filter((recipe) => recipe.planning?.supported === false)

  const nonPlannableRecipesByMachine = nonPlannableRecipes.reduce<Record<string, number>>(
    (counts, recipe) => {
      counts[recipe.machine.id] = (counts[recipe.machine.id] ?? 0) + 1
  ...
  const nonPositiveDurationRecipes = nonPlannableRecipes.filter(
    (recipe) => recipe.durationTicks <= 0 || recipe.durationSeconds <= 0,
  ).length

  const suspectedDurationOverflowRecipes = nonPlannableRecipes.filter((recipe) =>
    recipe.planning?.issues.includes('duration-overflow-suspected'),
  ).length

  const suspectedSentinelDurationRecipes = nonPlannableRecipes.filter((recipe) =>
    recipe.planning?.issues.includes('sentinel-duration-suspected'),
  ).length

  const sampleNonPlannableRecipes = nonPlannableRecipes.slice(0, 25).map((recipe) => {
    const issues = recipe.planning?.issues.join(', ') ?? ''
