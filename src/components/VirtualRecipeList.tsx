...
export function VirtualRecipeList({
  ...
  const getItemKey = useCallback((index: number) => recipes[index]?.id ?? index, [recipes])
  ...
  const rowVirtualizer = useVirtualizer({
    ...
    estimateSize: () => ESTIMATED_RECIPE_CARD_HEIGHT,
    getItemKey,
    gap: RECIPE_CARD_GAP,
  ...
  }, [recipes, rowVirtualizer])

  return (
    <div
      ref={scrollElementRef}
      className={['recipe-list', 'virtual-recipe-list', className].filter(Boolean).join(' ')}
    >
      <div
        className={'virtual-recipe-list-inner'}
        style={{
