...
import { formatDecimal, formatNumber, formatStackCompact } from '../lib/recipeHelpers'
...
export function PlannerGraphPreview({
  ...
  const outputNodes = graph.nodes.filter(
    (node) => node.status === 'target-output' || node.status === 'byproduct',
  )
    ...
    <section className="planner-graph-preview" aria-label="Planner graph preview">
      <div className="planner-preview-heading">
        <div>
          ...
          <p>
            Main-screen preview for one recipe. Machine count math here uses base recipe timing
            only; full tier and multiblock behavior belong in planner workspaces later.
          </p>
        </div>
          ...
          <label className="target-output-field">
            ...
            >
              {recipe.outputs.map((output, index) => (
                <option key={`${output.kind}:${output.id}:${output.meta}:${index}`} value={index}>
  ...
  return (
    <div className="planner-preview-metrics">
      {summary.targetOutput && (
        <span>
          Target: <strong>{formatStackCompact(summary.targetOutput)}</strong>
        </span>
      )}
      ...
      {summary.baseTargetRatePerSecond !== undefined && summary.targetOutput && (
        <span>
          Base rate:{' '}
          <strong>
            {formatDecimal(summary.baseTargetRatePerSecond)} {summary.targetOutput.unit}/s
          </strong>
        </span>
      )}
      ...
      {summary.operationsPerSecond !== undefined && (
        <span>
          Recipe cycles: <strong>{formatDecimal(summary.operationsPerSecond)} / sec</strong>
        </span>
      )}
      ...
      {summary.machineCountMode === 'fixed' ? (
        ...
        <span>
          Machine mode: <strong>Auto calculate</strong>
        </span>
      )}
      ...
      {summary.powerEstimate ? (
        <>
          <span>
            Recipe draw: <strong>{formatNumber(summary.powerEstimate.recipeEuPerTick)} EU/t</strong>
          </span>
          <span>
            Exact draw: <strong>{formatDecimal(summary.powerEstimate.exactEuPerTick)} EU/t</strong>
          </span>
          <span>
            Build draw: <strong>{formatNumber(summary.powerEstimate.roundedEuPerTick)} EU/t</strong>
          </span>
        </>
      ) : (
        <span>
          Power:{' '}
          <strong>{summary.usesEuPower ? 'Unavailable' : 'Not EU-powered / fuel-based'}</strong>
        </span>
      )}
    </div>
  )
...
function PlannerGraphNodeCard({
  ...
  return (
    ...
    >
      <div>
        <span className="planner-graph-node-kind">{getNodeKindLabel(node, stackRate)}</span>
      ...
      {stackRate && <PlannerStackRateSummary stackRate={stackRate} />}
      ...
      {stackAction && (
        ...
        >
          <button className="stack-action-button" type="button" onClick={stackAction.onClick}>
            {stackAction.label}
          </button>
        </div>
      )}
  ...
  return (
    <dl className="planner-graph-node-metrics planner-graph-rate-summary">
      ...
      {stackRate.constraintRatePerSecond !== undefined && (
        <div>
          <dt>
            {stackRate.constraintRole === 'input-provided' ? 'Provided / sec' : 'Wanted / sec'}
          </dt>
          <dd>
...
function PlannerStackConstraintControl({
  stackRate,
  solveMode,
  onStackConstraintRateChange,
}: PlannerStackConstraintControlProps) {
  const [inputDraft, setInputDraft] = useState<string | undefined>()

  const constraintRole: PlannerStackConstraintRole =
    stackRate.role === 'input' ? 'input-provided' : 'output-wanted'

  const isActiveSolveMode =
    stackRate.role === 'input' ? solveMode === 'from-inputs' : solveMode === 'from-outputs'
    ...
    >
      <label className="planner-stack-rate-control">
        ...
        <input
          ...
          type="number"
          value={currentValue}
          placeholder={isActiveSolveMode ? 'optional' : 'saved, but inactive in this solve mode'}
          onBlur={() => {
            const parsedValue = parseOptionalPositiveNumber(currentValue)

            setInputDraft(undefined)
            onStackConstraintRateChange(stackRate.stackKey, constraintRole, parsedValue)
          }}
          onChange={(event) => {
